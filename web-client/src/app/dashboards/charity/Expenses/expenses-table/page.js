...
export default function IncomesTable() {
  ...
  const handleDelete = async (id) => {
    try {
      await deleteExpense(id, userToken);
        ...
        <tbody>
          {incomes.map((income) => (
            ...
            >
              ...
              <td className="p-4">
                <Typography variant="small" className="font-normal">
                  {income.type === "OPERATIONAL" && "تشغيلية"}
                  {income.type === "NON_OPERATIONAL" && "غير تشغيلية"}
                  {income.type === "ADMINISTRATIVE" && "ادارية"}
                </Typography>
              </td>
              ...
              <td className="p-4">
                <div className="flex gap-2 ">
                  <Link
                    href={`/dashboards/charity/Expenses/expenses-table/${income.id}`}
