...
export default function NewDonorsOverTime() {
  const { userToken } = useCredentials();
  const [chartType, setChartType] = useState("bar"); // تبديل بين BarChart و LineChart
  const [viewType, setViewType] = useState("monthly"); // العرض حسب الأشهر أو الأيام
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const res = await axios.get(
        `${API_ENDPOINTS.DASHBOARDS.executiveSummary}/number-of-new-donors?${queryParams}`,
      ...
      setState({
        data: [],
        loading: false,
        error: "فشل تحميل بيانات المتبرعين الجدد",
      });
  ...
  return (
    <FinancialChart
      action={
        <ButtonGroup size="sm" variant="outlined" color="green">
      ...
      title="عدد المتبرعين الجدد عبر الفترات"
      description="يمكنك عرض البيانات حسب الأيام أو الأشهر."
