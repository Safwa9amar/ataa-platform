...
export default function PercentageOfCompletedPrograms() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const res = await axios.get(
        `${API_ENDPOINTS.DASHBOARDS.executiveSummary}/percentage-of-completed-programs`,
      ...
      setState({
        data: [],
        loading: false,
        error: "فشل تحميل بيانات نسبة إنجاز البرامج",
      });
    ...
    <FinancialChart
      action={
        <ButtonGroup color="green" variant="outlined" size="sm">
          ...
          <Button onClick={handleRetry} className="dark:text-white">
            <IoReload className="w-5 h-5" />
          </Button>
        </ButtonGroup>
      }
      title="نسبة البرامج المكتملة"
      description="يعرض هذا المخطط نسبة البرامج التي تم إكمالها من إجمالي البرامج المنفذة."
