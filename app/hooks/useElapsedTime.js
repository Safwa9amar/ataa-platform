...
const useElapsedTime = (createdAt) => {
  ...
  let customText;
  if (minutesElapsed < 1) {
    customText = `منذ ${secondsElapsed} ثانية`;
  } else if (hoursElapsed < 1) {
    customText = ` منذ ${minutesElapsed} دقائق`;
  } else if (hoursElapsed < 24) {
    customText = ` منذ ${hoursElapsed} ساعات`;
  } else if (daysElapsed < 7) {
    customText = ` منذ ${daysElapsed} يوم`;
  } else if (weeksElapsed < 4) {
    customText = `منذ ${weeksElapsed} اسبوع`;
  } else if (monthsElapsed < 12) {
    customText = `نذ ${monthsElapsed} شهر`;
  } else if (yearsElapsed >= 1) {
    customText = `منذ ${yearsElapsed}  سنة`;
  } else {
    customText = `اخر عملية تبرع منذ : ${createdDate.format("YYYY-MM-DD")}`;
  }
