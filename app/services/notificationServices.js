...
export const getNotificationsByUserId = async (userId, keywords, userToken) => {
  try {
    const response = await axios.get(
      `${API_ENDPOINTS.NOTIFICATION}/${userId}/?keywords=${keywords}`,
      {
        headers: getCommonHeaders(userToken),
      }
    );
...
export const markAllNotificationAsRead = async (userToken) => {
  try {
    const response = await axios.patch(
      `${API_ENDPOINTS.NOTIFICATION}/read-all`,
      {},
      {
        headers: getCommonHeaders(userToken),
      }
