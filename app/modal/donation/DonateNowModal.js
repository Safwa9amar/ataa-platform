    ...
    <KeyboardAvoidingView behavior="position" keyboardVerticalOffset={50}>
      <ScrollView contentContainerStyle={styles.container(theme.mangoBlack)}>
        <AnimatedCampaignsCard
          ...
          label={data?.title}
          description={
            data.donationType === CONSTANTS.DONATION_TYPES.ZAKAT
              ? "المبلغ الاجالي للزكاة : " + data?.cartTotalAmount + " دج"
              : data.donationType === CONSTANTS.DONATION_TYPES.CAMPAIGN
              ? "الرقم التعريفي للحملة #" + data?.id
              : data?.cartData
              ? data?.cartTotal
              : data?.fieldTitle +
                " | " +
        ...
        />
        <ContactDivider label="تبرع الان" />
        ...
        <TextInput
          ...
          style={[
            styles.textInput,
            shadowStyles,
            { backgroundColor: theme.borderColor, color: theme.textColor },
          ]}
        ...
        {!disbaleDonateBtn && (
          <PrimaryBtn onPress={handleDonateNow} title="تبرع الان" />
        )}
      </ScrollView>
    </KeyboardAvoidingView>
  );
